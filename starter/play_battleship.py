...
def get_valid_filename(msg: str) -> str:
    """
    Prompts for user input using the message msg.
    Returns the string input by the user if it is a valid filename.
    The file indicated by filename should exist in the same directory as
    this Python script. The function will continue prompting
...
def read_fleet_grid(game_file: TextIO) -> List[List[str]]:
    """
    Return the fleet grid that is found in game_file, with
    the format as described in the assignmend handout.
    """
...
def is_valid_game(fleet_grid: List[List[str]], ships: List[str], \
                  sizes: List[int]) -> bool:
...
def validate_game_parameters(grid: List[List[str]], ships: List[str],\
                             sizes: List[int]) -> bool:
    """ 
...
def get_valid_player_move(target_grid: List[List[str]]) -> List[int]:
    ...
    Completes error checking on valid bounds on moves using
    target_grid.
    ...
    while (not bf.is_valid_cell(row, col, grid_size) or 
...
def make_move(row: int, col: int, fleet_grid: List[List[str]], \
              ships: List[str], sizes: List[int], hits_list: List[int], \
